...
int wpa_debug_reopen_file(void)
{
	...
	if (last_path) {
		...
		wpa_printf(MSG_ERROR, "Last-path was not set, cannot re-open\n"
			"log file.");
		rv = -1;
	}
	return rv;
...
int wpa_debug_open_file(const char *path)
{
	...
	if (out_file == NULL) {
		wpa_printf(MSG_ERROR, "%s: Failed to open output file,\n"
			"using standard output", __func__);
		return -1;
	}
#ifndef _WIN32
...
void wpa_msg_global_ctrl(void *ctx, int level, const char *fmt, ...)
{
	...
	buf = os_malloc(buflen);
	if (buf == NULL) {
		wpa_printf(MSG_ERROR, "%s: Failed to allocate\n"
			"message buffer", __func__);
		return;
	}
	va_start(ap, fmt);
	len = vsnprintf(buf, buflen, fmt, ap);
